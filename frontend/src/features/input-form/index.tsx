...
export default function InputFormPage() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const payload = { ...form, fixed_segments: [] }
    ...
    <div className="min-h-screen bg-[#0f1117] py-12 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <form
          ...
          className="bg-[#13151f] border border-[#1e2133] rounded-xl p-8 space-y-5"
        >
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="bg-[#2563eb] hover:bg-[#1d4ed8] disabled:opacity-60 disabled:cursor-not-allowed text-white w-full py-3 rounded-lg font-medium transition-colors mt-2"
          >
            {isLoading ? 'Generating…' : 'Generate run-sheet with AI'}
          </button>
        </form>
      </div>
    </div>
